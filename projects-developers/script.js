...
function startLoader() {
  initOwner(); // تجهيز حساب الأونر فوراً
...
async function handleLogin() {
  ...
  await sleep(800); // محاكاة فحص الـ AI
  ...
  if (!user) {
    ...
    showErr(errEl, '<i class="fa-solid fa-circle-xmark"></i> البريد الإلكتروني أو كلمة المرور غير صحيحة');
...
async function handleRegister() {
  ...
  if (pass !== passC) {
    showErr(errEl, '<i class="fa-solid fa-circle-xmark"></i> كلمة المرور غير متطابقة');
    return;
  }
  if (pass.length < 6) {
    showErr(errEl, '<i class="fa-solid fa-circle-xmark"></i> كلمة المرور يجب أن تكون 6 أحرف على الأقل');
    return;
  }
  if (email === OWNER.email) {
    showErr(errEl, '<i class="fa-solid fa-circle-xmark"></i> هذا البريد غير متاح للتسجيل');
    return;
  }

  const users = DB.get('pb_users') || [];
  if (users.find(u => u.email === email)) {
    showErr(errEl, '<i class="fa-solid fa-circle-xmark"></i> البريد الإلكتروني مسجل بالفعل');
    return;
  }
  ...
  sucEl.innerHTML = `<i class="fa-solid fa-check-double"></i> ${aiMessage}! يمكنك الآن تسجيل الدخول.`;
...
function toggleUserSidebar() { document.getElementById('userSidebar').classList.toggle('open'); }
function closeUserSidebar() { if (window.innerWidth <= 768) document.getElementById('userSidebar').classList.remove('open'); }
...
const TYPE_ICONS = {
  ...
  extension: '<i class="fa-solid fa-puzzle-piece"></i>',
...
function renderProjects() {
  ...
  grid.innerHTML = projects.map((p, i) => `
    <div class="project-card" id="proj-${i}">
      <div class="pc-header">
        <div class="pc-icon">${TYPE_ICONS[p.type] || '<i class="fa-solid fa-box-open"></i>'}</div>
        <div>
...
async function handleUpload() {
  ...
  if (selectedFile) {
    scanText.innerHTML = '<i class="fa-solid fa-robot spin"></i> AI يفحص الملف بحثاً عن التهديدات...';
    await sleep(600);
    scanText.innerHTML = '<i class="fa-solid fa-magnifying-glass-chart"></i> تحليل الكود والبيانات...';
    await sleep(600);
    ...
    scanText.innerHTML = '<i class="fa-solid fa-file-lines"></i> فحص البيانات النصية...';
    await sleep(800);
  }
  ...
  sucEl.innerHTML = `<i class="fa-solid fa-check-double"></i> تم نشر المشروع "${escHtml(name)}" بنجاح! AI وافق عليه.`;
  sucEl.classList.remove('hidden');
...
async function runAIScan() {
  ...
  const lines = [
    ...
    { t: '<i class="fa-solid fa-triangle-exclamation"></i> البحث عن التهديدات...', c: 'st-warn', d: 1200 },
    { t: 'فحص الكود المشبوه...', c: 'st-wait', d: 1600 },
    { t: 'مراجعة قاعدة الفيروسات...', c: 'st-ok', d: 2000 },
    { t: '<i class="fa-solid fa-check"></i> لا تهديدات موجودة', c: 'st-ok', d: 2600 },
    { t: '<i class="fa-solid fa-shield-check"></i> النظام آمن', c: 'st-ok', d: 3000 },
  ...
  try {
    ...
    div2.innerHTML = `<span class="st-prompt">AI</span><span class="st-ok"><i class="fa-solid fa-shield-halved"></i> ${text.substring(0, 120)}</span>`;
    terminal.appendChild(div2);
  } catch {
    const div2 = document.createElement('div');
    div2.className = 'st-line';
    div2.innerHTML = `<span class="st-prompt">AI</span><span class="st-ok"><i class="fa-solid fa-shield-check"></i> النظام آمن — لا تهديدات</span>`;
    terminal.appendChild(div2);
  }
...
function renderDevs() {
  ...
  const statEl = document.getElementById('statDevs');
  if (statEl) statEl.textContent = devs.length;
  ...
  list.innerHTML = devs.map(d => `
    <div class="activity-item">
      <span class="ai-icon" style="color:var(--accent2)"><i class="fa-solid fa-code"></i></span>
...
function clearAllData() {
  ...
  const keys = ['pb_projects', 'pb_activity', 'pb_stats', 'pb_devs', 'pd_devs', 'pd_projs', 'pd_pending_sync'];
